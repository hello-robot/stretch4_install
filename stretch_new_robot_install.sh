...
while getopts ":fuhgm:i:H:" opt; do
    case $opt in
        h)
            show_help
            ;;
...
if [[ -n "$SETUP_FLEET_ID" ]]; then
    if [[ ! $SETUP_FLEET_ID =~ ^stretch-(re1|re2|se3|se4)-[0-9]{4}$ ]]; then
        echo "Error: Fleet ID '$SETUP_FLEET_ID' does not match required format stretch-(re1|re2|se3|se4)-XXXX."
...
timestamp='stretch4_install_'`date '+%Y%m%d%H%M'`;
...
logfile_update="$logdir/stretch_system_update.txt"
logfile_golden_image="$logdir/stretch_fleet_configure.txt"
...
if $do_update; then
    ...
    echo ""
    cd $HOME/stretch4_install/factory/$factory_osdir
    ./stretch_fleet_configure.sh -l $logdir |& tee $logfile_golden_image
    if [ $? -ne 0 ]; then
        echo_failure_help
    fi
else
    # Default: Run initial setup + system install + user install
    cd $HOME/stretch4_install/factory/$factory_osdir
...
echo "Please power cycle your robot"
# TODO: echo "Perform post install steps: https://docs.hello-robot.com/0.3/installation/robot_install/#post-install-steps"
echo "#############################################"
echo ""
