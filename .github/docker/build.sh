...
# Default values
FLEET_ID="${FLEET_ID:-stretch-se4-0000}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  -f, --fleet-id ID    Set the fleet ID (default: stretch-se4-0000)"
            ...
            echo "Examples:"
            echo "  $0"
            echo "  $0 -f stretch-se4-0001 -n my-stretch-install"
            echo "  FLEET_ID=stretch-se4-5678 $0"
            exit 0
            ;;
        *)
...
if [[ ! $FLEET_ID =~ ^stretch-(se4)-[0-9]{4}$ ]]; then
    print_error "Invalid fleet ID format: $FLEET_ID"
    echo "Fleet ID must match the format: stretch-(se4)-XXXX"
