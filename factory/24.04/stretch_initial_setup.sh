...
if [[ -n "$SETUP_FLEET_ID" ]]; then
    ...
    select model in stretch-re1 stretch-re2 stretch-se3 stretch-se4
    do
...
echo "Plug in charger before proceeding."
...
if [[ ! -d "$HOME/stretch4_install" ]]; then
    echo "Expecting Stretch4 Install to be in home folder. Exiting."
    exit 1
fi
...
if $do_factory_install; then
    ...
    cd ~/
    git clone https://github.com/hello-robot/stretch_fleet_ii.git ~/stretch_fleet
...
sudo sh -c "cp /etc/hello-robot/$HELLO_FLEET_ID/udev/*.rules /etc/udev/rules.d" #https://superuser.com/a/918334
sudo udevadm control --reload

echo "Allow powerprofilesctl and shutdown without password..."
...
echo "Configuring wired network for Lidar Networking..."
# Detect the first wired interface (excludes wifi/loopback)
ETH_IFACE=$(ip -o link show | awk -F': ' '{print $2}' | grep -E '^(en|eth)' | head -n 1)

# Remove existing TYPE=ethernet connections
sudo nmcli --terse --fields UUID,TYPE connection show | grep :802-3-ethernet | cut -d: -f1 | while read uuid; do
    sudo nmcli connection delete "$uuid" &> /dev/null
done

# Generate the Netplan config with the detected interface
# /32 mask prevents routing conflicts; specific routes enable the Lidars
sudo bash -c "cat > /etc/netplan/99-stretch.yaml <<EOF
network:
  version: 2
  ...
  ethernets:
    $ETH_IFACE:
      networkmanager:
        name: 'Stretch Internal Networking'
      dhcp4: no
      addresses:
        - 192.168.1.2/32
      routes:
        - to: 192.168.1.201/32 # right lidar
          scope: link
        - to: 192.168.1.202/32 # left lidar
          scope: link
        - to: 192.168.1.101/32 # Jetson
          scope: link
EOF"

# Secure and Apply
sudo chmod 600 /etc/netplan/99-stretch.yaml
